...
def _draw_keypoint(
    ...
    # Debug: Print keypoint data for debugging
    print(f"🔍 调试关键点数据:")
    print(f"  关键点总数: {len(keypoints)}")
    for kp_name, kp_coords in keypoints.items():
        print(f"    {kp_name}: {kp_coords} (类型: {type(kp_coords)})")
    
    print(f"  骨架连接数: {len(connections)}")
    ...
    for connection in connections:
        ...
        if (
            ...
            if (
                ...
                print(f"  ✅ 绘制连接: {kp1_name} -> {kp2_name}")
                valid_connections += 1
            else:
                print(f"  ❌ 格式错误: {kp1_name} -> {kp2_name} (格式: {type(kp1_coords)}, {type(kp2_coords)})")
                invalid_connections += 1
        else:
            # Debug why connection failed
            if kp1_name not in keypoints:
                print(f"  ❌ 缺失关键点: {kp1_name}")
            elif kp2_name not in keypoints:
                print(f"  ❌ 缺失关键点: {kp2_name}")
            elif keypoints[kp1_name] == "unvisible":
                print(f"  ❌ 关键点不可见: {kp1_name} = {keypoints[kp1_name]}")
            elif keypoints[kp2_name] == "unvisible":
                print(f"  ❌ 关键点不可见: {kp2_name} = {keypoints[kp2_name]}")
            invalid_connections += 1
    
    print(f"  有效连接: {valid_connections}, 无效连接: {invalid_connections}")
    
    # Check head keypoints specifically
    head_keypoints = ["nose", "left eye", "right eye", "left ear", "right ear"]
    visible_head = []
    invisible_head = []
    for kp in head_keypoints:
        if kp in keypoints:
            if keypoints[kp] != "unvisible":
                visible_head.append(kp)
            else:
                invisible_head.append(kp)
    
    print(f"  头部关键点 - 可见: {visible_head}, 不可见: {invisible_head}")
